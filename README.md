...
- 底部为纯白输入药丸，支持 1–4 行输入；投影只作用于整个输入药丸，内部按钮不单独投影。
- 输入框根据 IME 动画目标立即切换边距与高度，避免键盘收起后的状态延迟；文本状态会切换语音/发送按钮。
- 点击空白处、手动收起输入法或应用进入后台时都会强制清除输入焦点，避免光标残留及返回前台自动弹出输入法。
- 标题胶囊先测量标题和副标题的实际像素宽度，再以弹簧动画显式改变胶囊宽度；文字与右侧圆弧之间保留“圆角半径 + 2px”的安全距离。
- 顶部玻璃控件和整个输入药丸使用零偏移、向四周扩散的环境投影，不使用 Material 风格的单向投影。
