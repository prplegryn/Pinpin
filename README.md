...
- 标题胶囊先测量标题和副标题的实际像素宽度，再以弹簧动画显式改变胶囊宽度；变长时先完成容器扩展再交叉淡入新标题，变短时先稳定文字再收拢容器，避免逐帧重排、裁切闪烁；文字与右侧圆弧之间保留“圆角半径 + 2px”的安全距离。
- 顶部玻璃控件和整个输入药丸使用零偏移、向四周扩散的低饱和冷蓝灰环境投影；输入药丸的投影位于尺寸动画裁剪层之外，不会再在四角留下黑块。
- 性能路径会复用 Backdrop 玻璃效果与轮廓缓存；标题宽度动画只触发布局而不逐帧重组文字，输入框只观察 IME 动画目标并用一个进度同步全部几何变化；流式 token 只重组消息列表的隔离作用域，不再连带顶部玻璃、抽屉和设置页面。
- 页面、抽屉、锚点菜单、底部面板、确认框、列表增删重排、选择态和发送/停止控件共用一套可打断运动参数；普通控件的按压缩放在绘制阶段完成，避免为了反馈触发重新布局。
...
- 菜单按钮打开模态侧边栏；顶部搜索会在 Room 中匹配标题、摘要和完整消息正文，底部提供角色切换和设置入口。
- 历史分为置顶、今天、昨天、最近 7 天和更早，组内按更新时间倒序；整行支持长按管理，可置顶、重命名或经确认后级联删除，列表变化带稳定 ID 动画。
- 会话与消息由 Room 持久化；当前会话和未发送草稿可在配置变化或系统回收进程后恢复。
- 消息支持复制全文；最后一条助手回复可重新生成，数据库会先验证目标仍是末条再原子移除旧回复，避免误删历史分支。代码围栏使用独立代码表面和横向滚动。
- 支持停止流式回复、保存部分结果、规范化网络错误和不重复用户消息的失败重试；鉴权或地址错误会直接引导回设置。
- 内置通用、写作、技术和自定义角色。角色属于会话，切换只影响之后的请求，不改写历史内容。

## 自定义 API

- 设置页可填写 API 地址、可选 bearer key、模型、温度、无数据超时、上下文条数和自定义角色说明；连接测试会发现服务返回的模型 ID 并提供选择。
- 可分别关闭流式显示或完全省略 `temperature`，兼容不支持 SSE 或拒绝温度参数的 OpenAI-compatible 服务。
