...
第八轮流水线告警检查修正：读取成功 release 的完整编译日志，迁移参数与新版 Room 签名对齐；剪贴板切换到 Compose 1.12 的 suspend `LocalClipboard`/`ClipEntry`；移除图形和状态流中的废弃或冗余调用。SDK 安装器与 GitHub artifact action 自身的弃用提示不属于应用源码。
